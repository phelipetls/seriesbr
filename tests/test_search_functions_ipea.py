...
class TestIPEASearch(unittest.TestCase):
    ...
    def test_search_without_name_equal_int(self):
        ...
        select_ = "?$select=SERCODIGO,SERNOME,PERNOME,UNINOME,TEMCODIGO,FNTNOME,SERNUMERICA"
        filter_ = "&$filter=TEMCODIGO eq 12 and contains(FNTNOME,'IBGE') and SERNUMERICA eq 1"
        self.assertEqual(test, self.base + select_ + filter_)

    def test_search_multiple_types_including_equal_string(self):
        test = ipea.search(SERNOME="Selic", TEMCODIGO=12, FNTNOME="IBGE", PAICODIGO="SAU")
        select_ = "?$select=SERCODIGO,SERNOME,PERNOME,UNINOME,TEMCODIGO,FNTNOME,PAICODIGO"
